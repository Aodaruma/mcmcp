...
- `excavate_tunnel`だけは、配送済みの正確な入口下段壁面と現在の入口姿勢を起点に、水平・幅1・高さ2の有限坑道範囲を認可する。直線は最大160マス、枝坑道は事前に固定した左右の枝と主坑道への戻りを含める。新規表面はこの範囲内で、現在の通常fog/LOSによる実観測後にだけ使用できる。配送済みstoreの偽追加、TTL延長、壁越し探索、範囲外の鉱脈追跡をしない。各破壊のserver ACK後に再観測し、移動前には現在の局所安全を再証明する。全区間で元deadline・距離・camera・break予算を共有し、未知のmutationは再送しない。坑道完成は確認済み破壊と経路の物理通過で判定し、回収は別のserver同期inventory証拠として記録する。掘った個数を回収個数に置き換えない。
