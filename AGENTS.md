...
- 有限長押しのtarget_guardは観測済み座標・面・ブロック種と現在のフォーカスレイを照合する。expected_state:nullは明示expected_blockを必須とし、完全state指定時は従来通り全property一致を要求する。nullを無対象入力やhidden propertyの推測に使わない。既定は対象不一致で厳密停止する。明示した反復モードだけは新規入力を抑止して同一対象の復帰を待つ。開始済みuse、時間由来の開始予算、解放条件は設計仕様書を正本とする。
