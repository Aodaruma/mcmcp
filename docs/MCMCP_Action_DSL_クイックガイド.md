...
既設の定位置型丸石製造機には、top-level専用の`operate_known_cobblestone_generator`を使います。currentな丸石面の`target` / `face` / 完全stateをコピーし、`minecraft:iron_pickaxe`、cobblestoneの絶対inventory目標、`max_breaks`（1〜64）、再生成待ち（1〜200 ticks）、運転期限を宣言します。通常は3〜5分（3600〜6000 ticks）を推奨し、明示上限は30分（36000 ticks）です。raw mouse holdは公開せず、既存の短いattack leaseを各cycleだけ更新します。ACK＋airを確認するたびにcheckpoint/effectを残し、air・再生成待ち中はattackを解放します。次cycle直前に同じstate / face / reach / toolを再検証し、Esc、OFF、health・画面・world/session・stationary条件の変化では即座に入力を解放して終了します。

近傍の敵対mob判定は助言ではなくruntimeの安全条件であり、現状では該当するとActionが失敗・再計画へ進み、Agent入力が解放されます。mob TT向けのproduction設計では、一体・一攻撃ごとの同意は採用せず、場所と運転条件を許可するzone-scoped attack leaseを使います。1回の物理ローカルGrantをworld session / dimension、Grant時の実player位置とsupportからRuntimeが導出するtightなstation bounds、kill-zone bounds、mob type allowlist、trusted Runtimeがmain-handのexact stackから導出した宣言済みattack-side-effect profile、item ID / enchantments / 攻撃効果に関係する不変componentsの内部fingerprint、policy / `operate_kill_zone` / Job hash、最大攻撃数、最小攻撃間隔、期限へ束縛します。LLMはstation boundsを任意指定できません。

初期実装では`consent_ref`を複数Actionで持ち回りません。物理Grant後は2400 active client tick（約2分）以内に、ref以外の意味内容が同じ`operate_kill_zone`要求を再送できます。refだけをcanonical binding hashから除外し、zone、types、item profile、回数、間隔、運転期限等を変更した再送は拒否します。ただし、このbinding mismatchではrefをconsume / revokeせず、元の正しい要求を2分以内に再送できます。Action開始時にrefをsingle-consumeし、その内部だけで後から湧いた複数mobへ最大N回攻撃します。一体ごとの再同意は不要ですが、Action終了後のref再利用もできません。この2分は再送・開始の猶予であり、開始後の運転期限は別枠で最長約30分です。OFF、Esc / emergency、world / session変更、endpoint fault、shutdownでは、未消費refも実行中leaseも直ちにrevokeします。

side-effect profileをitem IDだけから推測せず、trusted Runtimeが実stackを宣言済みtable / adapterへ照合します。未知のMOD / data-driven profileはadapter追加まで拒否します。raw NBT / componentsや内部fingerprintはwire / UIへ出さず、UIにはsanitizedなitem IDと攻撃effect profileへ束縛済みであることだけを表示します。耐久damageの増減は許可するためXP TTのMendingを妨げず、同じitem ID / profileのstack交換も安全上は許可します。profile変化または武器の破損・消失で終了します。

`operate_kill_zone`は、その攻撃時点でfreshな`entity_ref`を内部で解決します。`entity_ref`はleaseのscope / hashへ含めず、毎回JITに解決して誤射防止とeffect追跡にだけ使います。敵の足元だけが見える形は安全なfixture候補ですが、見た目だけでは許可しません。station / zone間の正のcontact marginと、Grant時に認可したsupport、barrier、開口部、collision shapeがload済みblockとして維持されていることも毎回JIT検査します。さらにRuntime導出stationへの固定、kill zone、type、main-hand profile、target全AABBのzone内包、LOS、reach、crosshair、許可zone / type外の別hostile、接触、projectile、火、落下、desyncを検査します。playerは常に攻撃禁止で、allowlist型でも全AABBがzone外のmobはhard hazardです。Vanilla剣でsweep候補がある場合は効果範囲内の全living entityをdispatch直前に列挙し、全件が非player、allowlist type、whole AABB zone内と証明できる場合だけ許可します。1件でも未知・player・不許可type・zone外なら拒否し、その他のAoEとadapter未登録のMOD / data-driven profileも拒否します。攻撃枠はdispatch直前に原子的にreserveし、その後の例外やeffect `unknown`でも返却しません。health低下は解除不能で、直ちに入力解放・攻撃中断・lease失効とし、`SAFETY_INTERRUPTED`に`health_before`、`health_current`、`health_delta`、`reason`を付けてLLMへ返します。LLMはユーザーへ報告し、安全回復後に必要な場合だけ新しいzone同意を求めます。
...
途中停止では、serverで移送を確認できた分を`CONFIRMED`、結果不明な最後のクリックを`UNKNOWN`として別々に返します。whole stackを分割しないため、確認済みprefixの後に残る個数予算へ収まるstackがなく、`TRANSFER_BATCH_GOAL_NOT_REACHED`でActionが失敗する場合もあります。failed / cancelled terminalでもCONFIRMED effectはrollbackされないため、effectsを一度だけ反映してください。`failure.recoverable=true`は元Actionのblind replay許可ではありません。`partial.resume_requires_reobservation=true`なら、UNKNOWNの有無にかかわらずfreshなstate / observationを取得し、絶対goalに対する残作業を新しいActionとして計画します。自動補充や選択外stack・成分の変化を検知した場合、残りのクリックを続けません。

Take/store can move up to 14 whole matching stacks in one owned menu. `max_transfer_count` limits this batch; the minimum count is an absolute destination goal. Because stacks are never split, a confirmed prefix can still end in `TRANSFER_BATCH_GOAL_NOT_REACHED`. Each click requires fresh server confirmation, followed by one final reopen. A failed or cancelled terminal does not roll back confirmed effects: apply them exactly once. `recoverable` does not authorize blind replay. When `partial.resume_requires_reobservation` is true, obtain fresh state/observation and plan a new Action, even when there is no unknown effect. Transfer counts follow `stack_policy`, while inspection totals combine every component variant of an item ID. Use `item_id_any_components` for an item-ID ledger; never overwrite that total with a `default_components_only` count.
...
`repeat_target:true` と有限の `duration_ticks` を明示すると、同じActionの中で対象の再生成を待てます。対象消失、MISS、entity、別の座標・面・block・stateへの照準中は新しい採掘・使用を出さず、同じ対象条件が戻ると再開します。照準を自動で動かしません。開始済みの弓・飲食等の使用は一時的な対象不一致だけでは解除せず、次の使用開始には改めて一致を要求します。
