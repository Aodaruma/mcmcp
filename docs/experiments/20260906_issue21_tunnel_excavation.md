...
- dev fixtureが一時的に所有する22チャンクと512 rays/tickを、終了・失敗・置換・shutdown時に元の値へ戻す。
...
- 対象コード / Validated code commit: `d5bab0a814d5ecf98e2e741cac13959e0ce8dac3`（latest main `805cd5321f907932bf9f53a172c8975dd7014698`を統合）。
- `gradlew test harnessTest adminBridgeTest verifyHarnessIsolation build --console=plain`: **PASS**。unit 1,303、harness 24、admin 21、計1,348件でfailure/error 0。
...
- 坑道専用mock: capability gate **PASS**、bounded acceptanceの正常/範囲外変更・別run・status差し替え拒否 **PASS**、renderer witness **13/13 PASS**。
