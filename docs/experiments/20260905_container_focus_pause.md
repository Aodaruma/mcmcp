...
利用者がF3＋Pを実行した後、検証担当タスクから単独inspect Actionで19件連続成功したと報告された。対象はx=158の奥の棚、z=-305のy66/67（2件）、z=-304のy64〜68（5件）、z=-308・-309・-313の各y64〜67（各4件）。初報には17件との記述もあったが、列挙された対象の合計は19件で、担当タスクも訂正している。

設定変更後は画面切り替え・screen_not_clearの失敗は発生せず、エンドストーン3,456個と安山岩3,456個も公開NODE_EVIDENCEで取得できたとの報告を受けた。フォーカス喪失時の自動メニューが原因だったという見立てを支持し、ラージチェスト集計上限の修正も実機で確認できた。ただし今回の追試は1件ずつ別Actionで実行したもので、同一Action内の連続inspect再試験とは区別する。

担当タスクの台帳では設定変更前に取得したz=-305のy64/65を合わせ、奥の棚21チェストを取得済み。調査側はゲーム操作を行わず、検証担当も設定やアイテムを変更していない。ここでは担当タスクから受領した結果を記録し、全19件のAction IDや内容を独自に再取得したものとは扱わない。
