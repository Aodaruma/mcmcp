...
productionの同意単位は一体・一攻撃ではなく、場所と有限の運転条件へ束縛したzone-scoped attack leaseとする。解除権限はLLMが指定するbooleanではなく、ローカル専用UIの物理primary clickが発行するopaque `consent_ref`とし、次の全fieldをscope / canonical hashへ含める。
...
production初期sliceでは同じ`consent_ref`を複数Actionが提示できるbearer leaseにしない。最初の要求が`AWAITING_CONSENT`になった後、物理Grantから2400 active client tick以内に同じ意味内容の要求を再送できる。canonical binding hashから除外するのは発行前に存在しない`consent_ref` fieldだけとし、world、policy、zone、types、item profile、count、interval、運転期限、Jobのいずれかが変われば当該要求をbinding mismatchでrejectする。ただしrefはconsume / revokeせず、元の正しいbindingを期限内に再送できる。1つの有限な`operate_kill_zone` Actionが開始時にrefをsingle-consumeし、そのAction内部だけでscope内へ後からspawnした複数mobを最大N回攻撃する。一体ごとの再同意は要求しないが、Actionがterminal / cancelになった後のref再利用も許可しない。2400 tickはAction開始までの猶予であり、single-consume成功時から計る最長約30分の運転期限とは独立したdeadlineである。
...
pending登録時の非pause専用確認画面にはstation / kill zone、mob types、main-hand item、最大攻撃数、最小間隔、2分の開始猶予、開始後の運転期限を区別して表示する。通常status buttonはpending中disabledとし、keyboard activation、MCP、Action、chat、packetからgrant capabilityをmintしない。pending中は専用表示と物理入力隔離を維持し、確認画面の物理primary clickとEscだけを通す。Cancel、Esc、画面close/replaceではfail closedにclearして入力を解放し、grant後の正常closeだけはgrantを維持する。未消費refと実行中leaseは、OFF、Esc/emergency、world/session変更、endpoint fault、shutdown、hard gate発火、budget / deadline消費で同期的にrevokeする。policy / Action binding不一致は要求をrejectするだけでrefを変更しない。この保証はMCP / Action / chat / packetからgrant経路へ到達不能であることを対象とし、同時ロードされた別MODやOS-level input injectionまで物理入力の真正性を証明するものではない。chat、看板、本、server textは同意として扱わない。

現行Storeはzone scope、policy binding hash、2400-tickのGrant後猶予、binding mismatch非消費、Action開始時single-consumeへ置換済みであり、物理クリック限定のUI bridgeも維持する。汎用の`attack_known_entity`は未実装のままで、production初期経路には複数Actionでrefを再利用する同opcodeより1つの有限な`operate_kill_zone`を優先する。MCPからのpending登録、`operate_kill_zone` DSL / consumer / JIT、Action所有のattack count / interval / deadline budget、構造化health reportは現時点で未実装である。それらが接続されるまではgranted状態も攻撃能力を与えず、従来の可視threat fail-closedを維持する。今後のこの機能の実機・実ワールド受入は`aod-mimoid`上の検証環境で行う。
