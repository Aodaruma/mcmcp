...
安全な手持ちは通常block-useに副作用のないhotbarの`Item`本体（素材・剣・ピッケル等）を優先し、次に空または同じ安全条件を満たすoffhand、最後に空きhotbarを選択する。containerからの`QUICK_MOVE`が最後の空きhotbarを埋める場合も、安定したoffhandがあれば同一Actionの読み戻しと次のcontainer開封へ使用できる。名前・耐久値・enchantmentの差分だけでは拒否しない。標準状態との全成分一致は安全性の証拠にならないため、実stackに`CONSUMABLE` / `EQUIPPABLE` / `BLOCKS_ATTACKS` / `KINETIC_WEAPON`があれば両手とも除外する。設置可能なblock、bucket、着火具、斧・シャベル・クワ等の独自useを持つsubclassも除外する。空きslotが必須なのは額縁表示品のremoveなど別途明記した操作であり、container開封には必須ではない。候補がないcontainer失敗では既存の`inventory_safe_open_hand_required` / `inventory_safe_open_hand_unavailable`に加え、固定診断`safe_open_hand=no_side_effect_free_hotbar_or_offhand_item`と`remedy=prepare_empty_or_safe_offhand_or_plain_material_or_safe_mining_tool`を返す。offhandを空にするか、通常の鉄インゴットやピッケル等をhotbarまたはoffhandへ1枠用意すればよく、持ち物全体を空にする必要はない。
