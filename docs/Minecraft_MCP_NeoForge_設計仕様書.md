...
安全な手持ちは空きhotbarを優先し、空きがなければ通常block-useに副作用のない`Item`本体（素材・剣・ピッケル等）を選択する。名前・耐久値・enchantmentの差分だけでは拒否しない。標準状態との全成分一致は安全性の証拠にならないため、実stackに`CONSUMABLE` / `EQUIPPABLE` / `BLOCKS_ATTACKS` / `KINETIC_WEAPON`があれば除外する。設置可能なblock、bucket、着火具、斧・シャベル・クワ等の独自useを持つsubclassは除外する。空きslotが必須なのは額縁表示品のremoveなど別途明記した操作であり、container開封には必須ではない。候補がないcontainer失敗では既存の`inventory_safe_open_hand_required` / `inventory_safe_open_hand_unavailable`に加え、固定診断`safe_open_hand=no_side_effect_free_hotbar_item`と`remedy=prepare_empty_hotbar_slot_or_plain_material_or_safe_mining_tool`を返す。通常の鉄インゴットやピッケル等をhotbarへ1枠用意すればよく、持ち物全体を空にする必要はない。
