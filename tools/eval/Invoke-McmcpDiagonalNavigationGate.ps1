...
# Deterministic capability smoke test in the diagonal74 fixture, not fresh LLM acceptance.
function Invoke-McmcpDiagonalNavigationGate {
    [void][IO.Directory]::CreateDirectory($ArtifactDirectory)
    $path=Join-Path $ArtifactDirectory 'diagonal-result.json'
    ...
    try {
        ...
        foreach($goal in @(@(-6,61,5),@(-5,61,6),@(-6,61,7),@(-7,61,6))) {
            ...
            if($record.Count -ne 1){
                Add-GateEvent -Event 'missing_diagonal_target' -Detail @{goal=$goal;records=$records;world=$state.world}
                throw 'Fresh diagonal navigation target unavailable'
            }
            $node=@{id='diagonal';op='navigate_to_known';target=$record[0].navigation_target;tolerance=0.20}
            $terminal=Invoke-ActionRequest -Request (New-PrimitiveRequest -Name 'diagonal_navigation' `
            ...
            if($after.world.health -lt $state.world.health -or
                ...
                throw 'Diagonal arrival or health mismatch'
            }
    ...
    $result=@{status=$(if($null -eq $failure -and $results.Count -eq 4){'passed'}else{'failed'})
    ...
    [IO.File]::WriteAllLines((Join-Path $ArtifactDirectory 'diagonal-events.jsonl'),
...
if(-not $LibraryOnly){
    ...
    Invoke-McmcpDiagonalNavigationGate | ConvertTo-Json -Depth 40
}
