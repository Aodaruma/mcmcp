...
床施工では到着時と設置直前の支持取得を、設置直前の1回にまとめます。完全な対象座標・stateに一致するCONFIRMED effectと成功terminalを台帳へ保存できた場合だけ、直後の重複表面取得を省きます。次の設置の支持は改めて配送し、effectや保存済み座標から可視証拠を作りません。一般の移動・帰還・補充・照明、結果不明時の厳密な照合、各Action開始前の状態・session確認は維持します。独自wrapperで`Move-BuildingTo`を上書きする場合、任意の`-ForFloorPlacement`を受け取り、床呼出だけ末尾の重複支持取得を省けます。通常呼出の確認は残してください。
...
`Test-McmcpBuilding.ps1`は128×128・4回転の経路、16,505件の確認receiptを含む保存・読込み、二重計上防止、破損、lockを確認します。`Test-McmcpBuildingRecovery.ps1`は開始応答喪失、nonce照合、受付前拒否、world変更、材料収支を検証します。`Test-McmcpBuildingPersistence.ps1`は実ファイルの共有競合、有限失敗、tmp cleanup、送信前と受付後の保存失敗を検証します。`Test-McmcpBuildingObservations.ps1`は連続床の支持配送、結果不明・失敗時の停止、Action直前の状態確認、一般移動の支持確認を通信なしで検証します。いずれも`tools/check-source.ps1`に含めています。
