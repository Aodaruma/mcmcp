...
final class FixturePhase4RouteBlocker {
    ...
    static final String COW_TAG = "mcmcp_phase4_route_blocker";
    ...
    static void onServerTick(ServerTickEvent.Post event) {
        ...
        if (state == State.WAITING_FOR_FIRST_COLUMN && firstColumnComplete()) {
            ...
            LOGGER.info("MCMCP fixture placed a neutral route blocker for {} ticks",
                    OCCUPANCY_TICKS);
            return;
        }
        if (state == State.OCCUPYING_SECOND_ROUTE && tick >= removeAtTick) {
            ...
            LOGGER.info("MCMCP fixture removed the neutral route blocker");
        }
    }
    ...
    private static void stopWithReason(String reason) {
        LOGGER.warn("MCMCP Phase 4 route blocker stopped: {}", reason);
        stop();
    }
