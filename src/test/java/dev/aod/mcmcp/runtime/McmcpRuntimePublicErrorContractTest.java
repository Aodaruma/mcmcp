...
class McmcpRuntimePublicErrorContractTest {
    ...
    void rendererWaitTimeoutHasAFixedDiagnosisButUnrelatedFailuresAreNotReclassified() {
        ...
        assertThat(ActionAdmission.mapAdmissionFailure(timeout, recovery).failure().code()).isEqualTo("server_busy");
        recovery.evaluate(dev.aod.mcmcp.agent.observation.DeliveredPolicyEvidenceStore.SurfaceLeaseStatus.VALID,
                10, 0, false);
        var reply = ActionAdmission.mapAdmissionFailure(timeout, recovery);
        ...
        assertThat(ActionAdmission.mapAdmissionFailure(new RejectedExecutionException("full"), recovery)
                .failure().code()).isEqualTo("server_busy");
        assertThat(ActionAdmission.mapAdmissionFailure(
    ...
    void onlyActiveWorkConflictsUseTaskBusy() {
        ...
        var routineBusy = RuntimeFailures.mapFailure(
    ...
    void budgetDiagnosticKeepsTheBoundedNonReflectiveComponentMessage() {
        ...
        var reply = RuntimeFailures.mapFailure(new ActionDslException(
    ...
    void admissionDiagnosticsSurvivePublicMappingWithoutChangingTheSafetyErrorContract() {
        ...
        for (var reason : ActionAdmission.AdmissionFenceFailure.values()) {
            var reply = RuntimeFailures.mapFailure(ActionAdmission.admissionPreflightFailure(reason));
    ...
    private static void assertFailure(Throwable cause, String code) {
        var reply = RuntimeFailures.mapFailure(cause);
