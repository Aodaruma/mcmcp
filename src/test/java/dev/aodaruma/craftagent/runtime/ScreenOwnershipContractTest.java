...
class ScreenOwnershipContractTest {
    ...
    void requiredMixinHooksExactPostHandoffOpenAndInboundFullContentMethods() throws Exception {
        var node = classNode(
                "/dev/aodaruma/craftagent/mixin/client/ClientContainerSignalsMixin.class");

        var open = method(node, "craftagent$expectedContainerOpen");
        ...
        var full = annotation("Lorg/spongepowered/asm/mixin/injection/Inject;",
                method(node, "craftagent$fullContainerContent"));
        ...
        var close = annotation("Lorg/spongepowered/asm/mixin/injection/Inject;",
                method(node, "craftagent$serverContainerClose"));
    ...
    void packetBridgeFingerprintsPayloadsAndNeverSamplesPredictedMenuItemValues() throws Exception {
        ...
        assertThat(calls)
                .contains("dev/aodaruma/craftagent/runtime/ContainerSyncSignals$StackFingerprint"
                        + "#fromServerPacket")
                .doesNotContain(
    ...
    void screenEventsPreserveManualOverrideAndGateOnlyTheExpectedOpening() throws Exception {
        var node = classNode("/dev/aodaruma/craftagent/CraftAgentMod.class");

        assertThat(invocations(method(node, "onScreenOpening")))
                .containsSubsequence(
                        "dev/aodaruma/craftagent/runtime/ScreenOwnershipSignals#allowScreenOpening",
                        "dev/aodaruma/craftagent/runtime/CraftAgentRuntime#onManualInput");
        assertThat(invocations(method(node, "onScreenClosing")))
                .contains("dev/aodaruma/craftagent/runtime/ScreenOwnershipSignals#onScreenClosing");
        for (String method : List.of(
                "onKeyInput", "onMouseButtonInput", "onMouseScrollInput")) {
            assertThat(invocations(method(node, method)))
                    .contains(
                            "dev/aodaruma/craftagent/runtime/ScreenOwnershipSignals#onManualInput",
                            "dev/aodaruma/craftagent/runtime/CraftAgentRuntime#onManualInput");
        }
    }
    ...
    void ownershipCleanupContainsNoSyntheticClickDispatchPath() throws Exception {
        var node = classNode(
                "/dev/aodaruma/craftagent/runtime/ScreenOwnershipSignals.class");
